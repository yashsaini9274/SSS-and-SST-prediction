...
def create_fit_prophet(df, date_col, target, yrs_forecast=5):
    ...
    preds_df = preds_df.rename(columns = {'yhat':'TEMP','ds':'DATE'})
    
    return preds_df

# Fit model
train = data[data['DEPTH'].between(0,6)]
preds_prophet = create_fit_prophet(train, 'DATE','TEMP',5)
...
model = SARIMAX(train['TEMP'],order=(0,1,1),seasonal_order=(2,1,0,12))
...
model_prophet = Prophet()  # Replace with your Prophet model
model_arima = result  # Replace with your ARIMA model

# Holt-Winters model
model_hw = ExponentialSmoothing(train['TEMP'], seasonal='add', seasonal_periods=12)
...
hw_df = pd.DataFrame({'DATE': hw_dates, 'TEMP': prediction_hw})
...
f1 = go.Figure(
    data = [
        go.Scatter(x=arima['DATE'], y=arima['TEMP'], name='ARIMA'),
        go.Scatter(x=prophet['DATE'], y=prophet['TEMP'], name='Prophet'),
        go.Scatter(x=holt_winters['DATE'], y=holt_winters['TEMP'], name='Holt-Winters'),
    ],
